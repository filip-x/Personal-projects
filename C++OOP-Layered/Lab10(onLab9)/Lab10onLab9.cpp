	...
	if (path != "")
	{
		...
		if (path_to_mylist != "")
		{
			...
			if (path_to_mylist.substr(path_to_mylist.rfind('.') + 1, 3) == "csv")
				mylist = new My_list_CSV(path_to_mylist);
			else if (path_to_mylist.substr(path_to_mylist.rfind('.') + 1, 3) == "htm")
				mylist = new My_list_html(path_to_mylist);
			else if (path_to_mylist == "-1")
				mylist = new My_list_InMemory;
			this->controller = new Service(repo, mylist);
...
void Lab10onLab9::mode_a()
{
	...
	QObject::connect(Form->undo_button, &QPushButton::clicked, this, &Lab10onLab9::undo_gui);
	QObject::connect(Form->redo_button, &QPushButton::clicked, this, &Lab10onLab9::redo_gui);
	auto CtrlZ = new QShortcut(QKeySequence("Ctrl+Z"), FormWindow);
	QObject::connect(CtrlZ, &QShortcut::activated, this, &Lab10onLab9::undo_gui);// crtl Z
	auto CtrlY = new QShortcut(QKeySequence("Ctrl+Y"),FormWindow);
	QObject::connect(CtrlY, &QShortcut::activated, this, &Lab10onLab9::redo_gui);// ctrl y
...
void Lab10onLab9::register_hawk_eye(Observator* new_eye)
{
	hawk_eyes.push_back(new_eye);
}
void Lab10onLab9::unregister_hawk_eye(Observator* existing_eye)
{
	auto NewEnd = std::remove(hawk_eyes.begin(), hawk_eyes.end(), existing_eye);
	hawk_eyes.erase(NewEnd, hawk_eyes.end());
}
void Lab10onLab9::notify_all()
{
	for (auto eye : hawk_eyes)
	{
		eye->update();
	}
}
...
{
	auto window = new My_list_window(*mylist);
	register_hawk_eye(window);
	QObject::connect(window, &QWidget::objectNameChanged, [=]() { 
		unregister_hawk_eye((Observator*)window);
		});
	/*
	auto Form = new Ui_ListMylistForm;
	QWidget* widget = new QWidget;
...
void Lab10onLab9::build_screen()
{
	...
	if (path_to_file == "-1" && path_to_mylist == "-1")
	{
		...
		mylist = new My_list_InMemory;
		controller = new Service(new Repository, mylist);
	}
}
